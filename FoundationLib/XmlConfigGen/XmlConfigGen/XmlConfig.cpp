...
int CXmlClassGenerator::parseNode( pugi::xml_node current, XmlConfigNode *configNode )
{
	...
	if(nodes.size()>0)
	{
		for(int i=0;i<nodes.size();i++)
		{
			...
			configNode->properties.push_back(property);
		}
	}
...
std::string CXmlClassGenerator::genWholeStructor()
{
	...
	lines.push_back(_typename + " " + m_config.name); // 生成 class/struct name 形式
	lines.push_back("{");
	lines.push_back("public:");
	lines.push_back("\t" + _typename + "();");
	if(structType==1)
		lines.push_back("\tvirtual ~" + _typename + "();");
	...
	for(i=0;i<(int)m_config.nodes.size();i++)
	{
		...
		node->generatedOperationLoad=genLoadOperation(node);
		node->generatedOperationSave=genSaveOperation(node);
	}
...
std::string CXmlClassGenerator::list2string(CValueList<std::string> lines, std::string saperator)
{
	std::string ret;
	if(lines.size()<=0);
		return ret;

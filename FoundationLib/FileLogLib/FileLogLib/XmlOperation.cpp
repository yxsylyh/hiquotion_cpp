...
CXmlOperation::CXmlOperation(std::string path) : m_root(NULL)
{
...
// 获取指定节点的文本
std::string CXmlOperation::getElementText(TiXmlElement *element)
{
	if(!m_isOpen)
		return "";
...
// 获取指定节点的指定属性名称的属性值
std::string CXmlOperation::getElementAttribute(TiXmlElement *element, std::string propertyName)
{
	if(!m_isOpen)
		return "";
	...
	std::string propertyValue=attribute;

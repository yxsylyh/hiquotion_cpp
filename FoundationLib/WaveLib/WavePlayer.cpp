...
void CWavePlayer::initWaveFormat(int channels, int bitPerSample, int samplePerSecond)
{
...
void CWavePlayer::initWaveFormat(WAVEFORMATEX waveFormat)
{
...
// 设置和获取音频音量
DWORD CWavePlayer::getWaveVolume()
{
	if(m_result)
		return -1;
...
void CWavePlayer::setWaveVolume(DWORD volValue)
{
	if(m_result)
		return;
...
void CWavePlayer::setBufferSize(int size)
{
...
BOOL CWavePlayer::addData(char *data, int length)
{
	if(!data)
		return FALSE;
